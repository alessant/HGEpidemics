...
# saving class for latex plto
df[!, :class] .= "basic"
df[df[!, :character] .== "Jon_Snow", :class] .= Ref("gold")
df[df[!, :character] .== "Daenerys_Targaryen", :class] .= Ref("gold")
df[df[!, :character] .== "Tyrion_Lannister", :class] .= Ref("gold")
class = df[!, :class]

open("$output_path/latex-got-data-unique.csv", "w+") do f
    write(f, "x,y,class\n")

    for i in 1:length(x)
        write(f, "$(x[i]),$(y[i]),$(class[i])\n")
    end
end

open("$output_path/latex-got-data.csv", "w+") do f
    write(f, "x,y,class\n")
