...
"""
function hrwr_sparse(h::Hypergraph; heweights=nothing, α =.85, ϵ = 1.0e-6, iter=100)
    H, W, Dᵥ = compute_v_matrices_sparse!(h; heweights=heweights)
    Wₑ, Dₑ = compute_he_matrices_sparse(h; heweights=heweights)
...
"""
function compute_v_matrices_sparse!(h; heweights=nothing)
    H = spzeros(nhv(h), nhe(h)) #fill(0, nhv(h), nhe(h))
    D = spzeros(nhv(h), nhv(h)) #fill(0.0, nhv(h), nhv(h))
    W = spzeros(nhv(h), nhe(h)) #fill(0.0, nhv(h), nhe(h))
...
"""
function compute_he_matrices_sparse(h; heweights=nothing)
    D = spzeros(nhe(h), nhe(h)) #fill(0.0, nhe(h), nhe(h))
    W = spzeros(nhe(h), nhe(h)) #fill(0.0, nhe(h), nhe(h))
...
function fast_inverse(A)
    d = diag(A)
    d = 1 ./ d

    FD = spzeros(size(A)[1], size(A)[2])
    FD[diagind(FD)] = d
