...
# 5.3 time varying
###########
sim = deserialize("src/experiments/immunization/got/results/aamas21/data/got/exp54.data")
_keys = ["global_54_$(p)" for p in range(15, stop=40, step=5)]
...
# PLOT
#labels = [L"No \quad intervention", L"Uniform", L"d(\mathcal{G})", L"pr(\mathcal{G})", L"e(\mathcal{G})", L"d(\mathcal{H})", L"pr(\mathcal{H})", L"e(\mathcal{H})"]
...
legend(labels, ncol=3, loc="best", bbox_to_anchor=(0.5, 0., 0.5, 0.78))#, loc="best", bbox_to_anchor=(0.5, 0., 0.5, 0.75)

tick_params(labelsize="large")
xticks(range(0, stop=length(unmitigated_data)-1), range(.15, stop=.45, step=.05), fontweight="roman", fontsize="large")

ylabel("Infected (%)", fontweight="roman", fontsize="x-large", labelpad=10)
xlabel("Immunized agents (%)", fontweight="roman", fontsize="x-large", labelpad=10)
